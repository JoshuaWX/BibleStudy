...
export function FloatingWindow({ ariaLabel, children, size, toolbarLabel }: FloatingWindowProps) {
  ...
  return (
    <div
      className={
        isExpanded
          ? "fixed inset-0 z-50 overflow-y-auto bg-[#f7f8fb]/78 px-3 py-4 backdrop-blur-md sm:px-6 sm:py-8"
          : "w-full"
      }
    >
      <div
      ...
      >
        <div
          className={`mx-auto w-full ${isExpanded ? sizeClasses[size].expanded : sizeClasses[size].normal} rounded-[2rem] border border-white/80 bg-white/82 p-4 shadow-[0_24px_80px_rgba(42,45,67,0.16)] backdrop-blur-xl transition-all duration-300 sm:p-6`}
        >
